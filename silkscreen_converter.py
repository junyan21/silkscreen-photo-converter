...
class SilkscreenConverter:
    ...
    def create_halftone_pattern(self, image, lines, angle, dot_shape='circle',
                                vector_output=False):
        """網点パターンを生成（ベクター出力対応）"""
        ...
        for y in range(0, height, dot_spacing):
            for x in range(0, width, dot_spacing):
                ...
                if dot_size > 0:
                    ...
                    # ラスター出力用描画
                    self._draw_dot(result_array, center_x, center_y, dot_size,
                                   angle_rad, dot_shape, width, height)

        return Image.fromarray(result_array)

    def _draw_dot(self, array, center_x, center_y, size, angle, shape,
                  width, height):
        """指定した形状の網点を描画"""
        radius = size // 2
        ...
        for y in range(y_start, y_end):
            for x in range(x_start, x_end):
                ...
                if shape == 'circle':
                    ...
                    draw_pixel = (abs(rotated_x) <= radius and
                                  abs(rotated_y) <= radius)
                elif shape == 'diamond':
                    draw_pixel = abs(rotated_x) + abs(rotated_y) <= radius
                elif shape == 'line':
                    draw_pixel = (abs(rotated_y) <= radius * 0.3 and
                                  abs(rotated_x) <= radius)
    ...
    def convert(self, input_path, output_path, lines=15, angle=45,
                dot_shape='circle', contrast=1.0, brightness=0,
                dpi=300, format_type='PNG'):
        """メイン変換処理"""
...
@click.option('--lines', default=15, type=int,
              help='線数 (10-30, デフォルト: 15)')
@click.option('--angle', default=45, type=int,
              help='網点角度 (0-90, デフォルト: 45)')
@click.option('--shape', 'dot_shape',
              type=click.Choice(['circle', 'square', 'diamond', 'line']),
              default='circle', help='網点形状 (デフォルト: circle)')
@click.option('--contrast', default=1.0, type=float,
              help='コントラスト調整 (0.5-2.0, デフォルト: 1.0)')
@click.option('--brightness', default=0, type=int,
              help='明度調整 (-50 to 50, デフォルト: 0)')
@click.option('--dpi', default=300, type=int,
              help='出力解像度 (デフォルト: 300)')
@click.option('--format', 'format_type',
              type=click.Choice(['PNG', 'TIFF', 'PDF', 'AI']),
              default='PNG', help='出力形式 (デフォルト: PNG)')
@click.option('--batch', is_flag=True,
              help='フォルダ内の全画像を一括変換')
def main(input_path, output_path, lines, angle, dot_shape, contrast,
         brightness, dpi, format_type, batch):
    """
    ...
    if batch:
        ...
        for file in image_files:
            ...
            try:
                converter.convert(input_file, output_file, lines, angle,
                                  dot_shape, contrast, brightness, dpi,
                                  format_type)
            except Exception as e:
    ...
    # 変換実行
    try:
        converter.convert(input_path, output_path, lines, angle, dot_shape,
                          contrast, brightness, dpi, format_type)
